...
import { renderTimelineChart, renderCustomChartsSection } from './summary/summaryChartUtils';
import { renderByDateView } from './summary/summaryByDateView';
import { renderBySessionView } from './summary/summaryBySessionView';
import { createSummaryState, resetSummaryState, SummaryState } from './summary/summaryState';
...
export async function renderSummary() {
  ...
  summaryState.allDailyData = await ipcRenderer.invoke('get-daily-summary', getCurrentUserId());
  ...
  function updateWeeklySummary() {
    ...
    const weekData = filterDailyDataForWeek(summaryState.allDailyData, summaryState.currentWeekMonday);
    weeklySummaryContainer.appendChild(renderTimelineChart(weekData, summaryState.currentWeekMonday));
  ...
  function attachWeekNavHandlers() {
    ...
    if (prevBtn) {
      prevBtn.onclick = () => {
        // Go to previous week
        summaryState.currentWeekMonday.setDate(summaryState.currentWeekMonday.getDate() - 7);
        updateWeeklySummary();
      };
    }
    if (nextBtn) {
  ...
  // Initialize views
  await renderByDateView(byDateContainer, summaryState.allDailyData, summaryState.byDateView);
