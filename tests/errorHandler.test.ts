...
import { classifyError, withRetry } from "../src/utils/errorHandler";
...
describe("errorHandler", () => {
  describe("classifyError", () => {
    ...
    it("classifies network errors correctly", () => {
      ...
      expect(
        classifyError({ message: "connect ECONNREFUSED 127.0.0.1:80" }),
      ).toBe("network");
    ...
    it("classifies unknown errors correctly", () => {
      expect(
        classifyError(new Error("Cannot read properties of undefined")),
      ).toBe("unknown");
      expect(classifyError({ message: "SyntaxError: Unexpected token" })).toBe(
        "unknown",
      );
  ...
  describe("withRetry", () => {
    const consoleApi = global["console"];
    const originalConsoleError = consoleApi.error;

    beforeAll(() => {
      consoleApi.error = vi.fn();
    });

    afterAll(() => {
      consoleApi.error = originalConsoleError;
    });

    // We can also spy on console.error directly inside specific tests
    // if we don't want to suppress it globally.
