...
import { DB_NAME, APP_NAME } from "../shared/constants";
...
function populateDummyUsers() {
  db.prepare(
    `INSERT OR IGNORE INTO users (id, username, avatar) VALUES (1, 'Default', ''), (2, 'Bob', '')`
  ).run();
}
...
function populateSessions(userId: number, days = 30) {
  for (let i = 0; i < days; ++i) {
    ...
    for (let j = 0; j < sessionCount; ++j) {
      ...
      const description =
        Math.random() > 0.5 ? "Some notes about this session." : null;
...
function populateTags(userId: number) {
  tags.forEach((tag, i) => {
    ...
    db.prepare(
      `INSERT OR IGNORE INTO tags (name, user_id, color) VALUES (?, ?, ?)`
    ).run(tag, userId, color);
  });
}
...
function assignTagsToSessions(userId: number) {
  const sessionRows = db
    ...
    .all(userId);
  const tagRows = db
    .prepare(`SELECT id FROM tags WHERE user_id = ?`)
