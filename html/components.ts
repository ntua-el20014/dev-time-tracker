...
export interface ChartConfig {
  ...
  groupBy: 'date' | 'language' | 'editor' | 'tag' | 'day-of-week' | 'hour-of-day';
...
export function renderCustomChart(
  ...
  const container = document.getElementById(containerId) as CustomChartContainer | null;
  if (!container) return;
  container.innerHTML = '';
  ...
  // Get theme colors
  const axisColor = getComputedStyle(document.body).getPropertyValue('--fg').trim() || '#333';
  const palette = ['#4f8cff', '#ffb347', '#7ed957', '#ff6961', '#b19cd9', '#f67280', '#355c7d', '#ff9999', '#66b3ff', '#99ff99'];
  ...
  const chartConfig = {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: function(ctx: { label?: string; parsed: number | { y: number } }) {
              const label = ctx.label || '';
              const value = typeof ctx.parsed === 'number' ? ctx.parsed : ctx.parsed.y;
              const unit = getValueUnit(config.aggregation);
              return `${label}: ${formatChartValue(value, config.aggregation)}${unit}`;
            }
          }
        }
      },
        ...
        y: {
          ...
          ticks: { 
            ...
            callback: function(value: string | number) {
              return formatChartValue(typeof value === 'string' ? parseFloat(value) : value, config.aggregation) + getValueUnit(config.aggregation);
            }
          }
        }
      } : undefined
...
function processChartData(data: (DailySummaryRow | SessionRow)[], config: ChartConfig) {
...
function getItemValue(item: DailySummaryRow | SessionRow, aggregation: string): number {
  switch (aggregation) {
    case 'total-time':
      return 'total_time' in item ? item.total_time : (item as SessionRow).duration || 0;
    case 'session-count':
...
export function showChartConfigModal(options: {
  data: (DailySummaryRow | SessionRow)[],
  dataSource: 'sessions' | 'daily-summary',
  onCreateChart: (config: ChartConfig, data: (DailySummaryRow | SessionRow)[]) => void
}) {
      ...
      <div class="chart-modal-body">
        <form id="chartConfigForm">
          <div class="form-group">
            ...
            <input type="text" id="chartTitle" class="chart-title-input" value="${options.dataSource === 'sessions' ? 'Session Analysis' : 'Daily Summary Analysis'}" required>
          </div>
          ...
          <div class="form-group">
            ...
            <select id="groupBy" required>
              ...
              ${options.dataSource === 'sessions' ? '<option value="tag">Tag</option>' : ''}
              <option value="day-of-week">Day of Week</option>
              ${options.dataSource === 'sessions' ? '<option value="hour-of-day">Hour of Day</option>' : ''}
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select id="aggregation" required>
              ...
              ${options.dataSource === 'sessions' ? '<option value="session-count">Number of Sessions</option>' : ''}
              ${options.dataSource === 'sessions' ? '<option value="average-duration">Average Session Duration</option>' : ''}
            </select>
          </div>
  ...
  const aggregationSelect = modal.querySelector('#aggregation') as HTMLSelectElement;
  ...
  [titleInput, typeSelect, groupBySelect, aggregationSelect].forEach(element => {
